...
# Fork patch: when iframe embedding is enabled, cookies must be SameSite=None +
# Secure so that the browser sends them in a cross-origin iframe (e.g. a Chrome
# extension sidepanel embedding this UI). Without this, the app loads but
# login/session calls return empty → user is stuck on splash/login screen. Users
# can still override these explicitly via env vars.
...
WEBUI_SESSION_COOKIE_SAME_SITE = os.getenv('WEBUI_SESSION_COOKIE_SAME_SITE', _IFRAME_COOKIE_SAME_SITE_DEFAULT)
WEBUI_SESSION_COOKIE_SECURE = os.getenv('WEBUI_SESSION_COOKIE_SECURE', _IFRAME_COOKIE_SECURE_DEFAULT).lower() == 'true'
WEBUI_AUTH_COOKIE_SAME_SITE = os.getenv('WEBUI_AUTH_COOKIE_SAME_SITE', WEBUI_SESSION_COOKIE_SAME_SITE)
WEBUI_AUTH_COOKIE_SECURE = (
    os.getenv(
        'WEBUI_AUTH_COOKIE_SECURE',
        os.getenv('WEBUI_SESSION_COOKIE_SECURE', _IFRAME_COOKIE_SECURE_DEFAULT),
    ).lower()
    == 'true'
)
