...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        RAG_USE_CONTEXT_CACHE: bool = Field(
            default=os.getenv("GOOGLE_RAG_USE_CONTEXT_CACHE", "false").lower()
            == "true",
    ...
    def _get_rag_part(self) -> str:
        ...
        if rag_url and (
            rag_url.startswith("http://") or rag_url.startswith("https://")
        ):
            try:
        ...
        if (
            ...
            and self.valves.RAG_DOCUMENT_CONTEXT_BASE64
            and self.valves.RAG_DOCUMENT_CONTEXT_BASE64.strip()
        ):
            try:
                ...
                self.log.warning(
                    f"RAG: failed to decode RAG_DOCUMENT_CONTEXT_BASE64: {e}"
                )
        if (
            not document_context
            and self.valves.RAG_DOCUMENT_CONTEXT
            and self.valves.RAG_DOCUMENT_CONTEXT.strip()
        ):
            document_context = self.valves.RAG_DOCUMENT_CONTEXT.strip()
            self.log.debug(
                f"RAG: using document context plain text ({len(document_context)} chars)"
            )
    ...
    @staticmethod
    def _rag_cache_expire_time_to_unix(
        expire_time: Any, fallback_ttl_seconds: int = 0
    ) -> Optional[float]:
    ...
    async def _recreate_rag_cache_after_invalid(
        ...
        if (
            ...
            or not getattr(self.valves, "RAG_USE_CONTEXT_CACHE", False)
            or not getattr(self.valves, "RAG_CACHE_TTL_SECONDS", 0)
        ):
            return None
        try:
            ...
            self._rag_cache_key = hashlib.sha256(
    ...
    def pipes(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if "API key" in msg or "INVALID_ARGUMENT" in msg:
                msg = "Google API key invalid or missing. Set GOOGLE_API_KEY env var or re-enter the key in pipeline settings (gear icon)."
...
{quoted_content}
    ...
    async def pipe(
        ...
        try:
            ...
            system_instruction_backup: Optional[str] = (
                None  # for RAG cache recreate on API error
            )
            ...
            if supports_image_generation:
                ...
                if (
                    ...
                    system_instruction_backup = (
                        system_instruction  # keep for recreate on API "cache expired"
                    )
                    ...
                    if cache_still_valid:
                        ...
                        try:
                            ...
                            self._rag_cache_expire_time = (
                                self._rag_cache_expire_time_to_unix(
                                    raw_expire, self.valves.RAG_CACHE_TTL_SECONDS
                                )
                            )
                            if self._rag_cache_expire_time is None:
            ...
            generation_config = self._configure_generation(
                ...
                model_id,
                cached_content=(
                    cached_content_name if not supports_image_generation else None
                ),
            )
