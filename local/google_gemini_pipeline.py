...
description: Google Gemini modely v Open WebUI – chat, obrázky, RAG (Vertex AI Search), grounding, tool calling. Použijte tento pipeline pro připojení Gemini přes API klíč z AI Studio.
features:
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        # RAG: znalostní báze jako „endpoint“ (jako v AI Studio – system_instruction + dokumenty)
        RAG_DOCUMENT_URL: str = Field(
            default=os.getenv("GOOGLE_RAG_DOCUMENT_URL", ""),
            description="URL na .txt (nebo jiný text). Pipeline obsah stáhne a použije jako znalostní báze. Např. GitHub raw, Google Drive (odkaz na stažení), vlastní server. Ideální pro Railway – soubory mohou být kdekoli.",
        )
        RAG_DOCUMENT_FILE_PATH: str = Field(
            default=os.getenv("GOOGLE_RAG_DOCUMENT_FILE_PATH", ""),
            description="Cesta k .txt souboru na serveru (sloučená znalostní báze). Např. /data/rag/merged.txt.",
        )
        RAG_DOCUMENT_CONTEXT: str = Field(
            default=os.getenv("GOOGLE_RAG_DOCUMENT_CONTEXT", ""),
            description="Znalostní báze (sloučený text). Model odpovídá POUZE z něj. "
            "Priorita: RAG_DOCUMENT_URL → RAG_DOCUMENT_FILE_PATH → toto pole → RAG_DOCUMENT_CONTEXT_BASE64.",
        )
        RAG_DOCUMENT_CONTEXT_BASE64: str = Field(
            default=os.getenv("GOOGLE_RAG_DOCUMENT_CONTEXT_BASE64", ""),
            description="Stejné jako RAG_DOCUMENT_CONTEXT, ale v Base64 (např. export z AI Studio). "
            "Při nastavení se dekóduje a použije jako kontext pro striktní RAG.",
        )
        RAG_SYSTEM_INSTRUCTION_PREFIX: str = Field(
            default=os.getenv(
                "GOOGLE_RAG_SYSTEM_INSTRUCTION_PREFIX",
                "Jsi expert na nahrané dokumenty. Odpovídej POUZE na základě přiloženého textu. "
                "Pokud informace v dokumentech chybí, řekni to.\n\n--- Znalostní báze ---\n",
            ),
            description="Text před vložením znalostní báze do system instruction (striktní RAG režim).",
        )
        RAG_USE_CONTEXT_CACHE: bool = Field(
            default=os.getenv("GOOGLE_RAG_USE_CONTEXT_CACHE", "false").lower() == "true",
            description="Použít Context Caching: dokumenty se pošlou Google jednou, dostanete cache ID, další dotazy jen ID + otázka. Rychlejší a levnější než posílat kontext pokaždé.",
        )
        RAG_CACHE_TTL_SECONDS: int = Field(
            default=int(os.getenv("GOOGLE_RAG_CACHE_TTL_SECONDS", "3600")),
            description="Jak dlouho má Google držet cache (v sekundách). Např. 3600 = 1 hodina. Po vypršení se vytvoří nová cache.",
        )
        ...
        MODEL_WHITELIST: str = Field(
            ...
            description="Comma-separated model IDs. Default: production models (excludes deprecated 3-pro, 2.0). "
            "Set empty or override via GOOGLE_MODEL_WHITELIST to show all.",
        )
    ...
    def _get_rag_part(self) -> str:
        """Vrátí jen RAG část (prefix + znalostní báze) pro Context Caching nebo vložení do promptu."""
        document_context = ""
        rag_url = (self.valves.RAG_DOCUMENT_URL or "").strip()
        if rag_url and (rag_url.startswith("http://") or rag_url.startswith("https://")):
            try:
                with urlopen(rag_url, timeout=30) as resp:
                    raw = resp.read()
                    if len(raw) > 50 * 1024 * 1024:  # max 50 MB
                        self.log.warning(
                            f"RAG: URL content too large ({len(raw)} bytes), skipping"
                        )
                    else:
                        document_context = raw.decode("utf-8", errors="replace")
                        self.log.debug(
                            f"RAG: using document context from URL ({len(document_context)} chars)"
                        )
            except (URLError, HTTPError, OSError) as e:
                self.log.warning(f"RAG: failed to fetch RAG_DOCUMENT_URL: {e}")
        file_path = (self.valves.RAG_DOCUMENT_FILE_PATH or "").strip()
        if not document_context and file_path:
            try:
                ...
                if os.path.isfile(path):
                    ...
                    self.log.debug(
                        f"RAG: using document context from file {path} ({len(document_context)} chars)"
                    )
                else:
                    self.log.warning(f"RAG: file not found or not a file: {path}")
            except Exception as e:
                self.log.warning(f"RAG: failed to read RAG_DOCUMENT_FILE_PATH: {e}")
        if not document_context and self.valves.RAG_DOCUMENT_CONTEXT_BASE64 and self.valves.RAG_DOCUMENT_CONTEXT_BASE64.strip():
            try:
                document_context = base64.b64decode(
                    self.valves.RAG_DOCUMENT_CONTEXT_BASE64.strip()
                ).decode("utf-8")
                self.log.debug(
                    f"RAG: using document context from Base64 ({len(document_context)} chars)"
                )
            except Exception as e:
                self.log.warning(f"RAG: failed to decode RAG_DOCUMENT_CONTEXT_BASE64: {e}")
        if not document_context and self.valves.RAG_DOCUMENT_CONTEXT and self.valves.RAG_DOCUMENT_CONTEXT.strip():
            document_context = self.valves.RAG_DOCUMENT_CONTEXT.strip()
            self.log.debug(f"RAG: using document context plain text ({len(document_context)} chars)")

        if document_context:
            return (
                (self.valves.RAG_SYSTEM_INSTRUCTION_PREFIX or "").strip()
                + "\n"
                + document_context
            )
        return ""

    def _combine_system_prompts_without_rag(
        self, user_system_prompt: Optional[str]
    ) -> Optional[str]:
        """Stejné jako _combine_system_prompts, ale bez RAG dokumentů (pro použití s Context Caching)."""
    ...
    def _combine_system_prompts(
        ...
        """Combine default system prompt with user-defined system prompt and optional RAG context."""
        rag_part = self._get_rag_part()
        default_prompt = self.valves.DEFAULT_SYSTEM_PROMPT.strip()
        if rag_part and default_prompt:
            default_prompt = f"{default_prompt}\n\n{rag_part}"
        elif rag_part:
            default_prompt = rag_part
        user_prompt = user_system_prompt.strip() if user_system_prompt else ""
        if default_prompt and user_prompt:
    ...
    def __init__(self):
        ...
        # RAG Context Caching (Google): cache name, expiry, hash of content
        self._rag_cache_name: Optional[str] = None
        self._rag_cache_expire_time: Optional[float] = None
        self._rag_cache_key: Optional[str] = None

    @staticmethod
    def _rag_cache_expire_time_to_unix(expire_time: Any, fallback_ttl_seconds: int = 0) -> Optional[float]:
        """Normalize Google cache expire_time to Unix timestamp (float). API may return datetime or seconds."""
        ...
        if isinstance(expire_time, (int, float)):
            t = float(expire_time)
            if t > 1e9:
                return t  # Unix timestamp (seconds; 1e9 = Sep 2001)
            if t > 0:
                return now + t  # TTL in seconds
            return None
        ...
        try:
            ...
            return now + fallback_ttl_seconds if fallback_ttl_seconds else None

    @staticmethod
    def _is_rag_cache_invalid_error(exc: BaseException) -> bool:
        """True if the exception indicates cached_content is expired/invalid (so we should recreate cache)."""
        msg = (getattr(exc, "message", None) or str(exc)).lower()
        if getattr(exc, "code", None) in (404, 400, 409):
            return True
        for needle in ("cached", "cache", "not found", "expired", "invalid", "no longer"):
            if needle in msg:
                return True
        return False

    async def _recreate_rag_cache_after_invalid(
        self, system_instruction: str, model_id: str
    ) -> Optional[str]:
        """Invalidate local RAG cache state and create a new one (e.g. after API returned cache expired). Returns new cache name or None."""
        self._rag_cache_name = None
        self._rag_cache_expire_time = None
        self._rag_cache_key = None
        if not system_instruction or not getattr(self.valves, "RAG_USE_CONTEXT_CACHE", False) or not getattr(self.valves, "RAG_CACHE_TTL_SECONDS", 0):
            return None
        try:
            client = self._get_client()
            cache_config = getattr(types, "CreateCachedContentConfig", None)
            if cache_config is None:
                return None
            config = cache_config(
                system_instruction=system_instruction,
                ttl=f"{self.valves.RAG_CACHE_TTL_SECONDS}s",
            )
            ...
            now = time.time()
            self._rag_cache_name = getattr(cache, "name", None)
            raw_expire = getattr(cache, "expire_time", None)
            self._rag_cache_expire_time = self._rag_cache_expire_time_to_unix(
                raw_expire, self.valves.RAG_CACHE_TTL_SECONDS
            )
            if self._rag_cache_expire_time is None:
                self._rag_cache_expire_time = now + self.valves.RAG_CACHE_TTL_SECONDS
            self._rag_cache_key = hashlib.sha256(system_instruction.encode()).hexdigest()
            self.log.debug(
                f"RAG: recreated context cache {self._rag_cache_name} after API error (e.g. expired)"
            )
            return self._rag_cache_name
        except Exception as e:
            self.log.warning(f"RAG: recreate cache after invalid failed: {e}")
            return None
    ...
    def get_google_models(self, force_refresh: bool = False) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # Apply MODEL_WHITELIST filter
            whitelist = (self.valves.MODEL_WHITELIST or "").strip()
            # Deprecated models to exclude when whitelist is empty (3-pro EOL 2026-03-09, 2.0 EOL 2026-06-01)
            ...
            if whitelist:
                ...
                # No whitelist: show gemini-* but exclude deprecated
                filtered_models = {
                    k: v
    ...
    def pipes(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # API key invalid or expired – avoid sending encrypted key to Google
            msg = str(e)
    ...
    def _configure_generation(
        ...
        Args:
            ...
            system_instruction: Optional system instruction string (None when using cached_content)
            enable_image_generation: Whether to enable image generation
            model_id: The model ID being used (for feature support checks)
            cached_content: Optional cached content resource name (RAG context cache)
        ...
        if cached_content:
            gen_config_params["cached_content"] = cached_content
            gen_config_params["system_instruction"] = None
...
{quoted_content}
    ...
    async def pipe(
        ...
        try:
            ...
            system_instruction_backup: Optional[str] = None  # for RAG cache recreate on API error
            ...
            if supports_image_generation:
                ...
                # RAG Context Caching: use Google cache so we send only cache ID, not full context.
                # Cache has TTL (RAG_CACHE_TTL_SECONDS); we treat it as dead 60s before expiry and recreate.
                if (
                    getattr(self.valves, "RAG_USE_CONTEXT_CACHE", False)
                    and getattr(self.valves, "RAG_CACHE_TTL_SECONDS", 0)
                    and self._get_rag_part()
                ):
                    system_instruction_backup = system_instruction  # keep for recreate on API "cache expired"
                    context_hash = hashlib.sha256(
                        (system_instruction or "").encode()
                    ).hexdigest()
                    now = time.time()
                    cache_still_valid = (
                        self._rag_cache_name
                        and self._rag_cache_key == context_hash
                        and self._rag_cache_expire_time is not None
                        and now < self._rag_cache_expire_time - 60
                    )
                    if cache_still_valid:
                        cached_content_name = self._rag_cache_name
                        system_instruction = None
                        self.log.debug(
                            f"RAG: using existing context cache {self._rag_cache_name}"
                        )
                    else:
                        try:
                            client = self._get_client()
                            cache_config = getattr(
                                types, "CreateCachedContentConfig", None
                            )
                            if cache_config is None:
                                raise AttributeError(
                                    "CreateCachedContentConfig not in types"
                                )
                            config = cache_config(
                                system_instruction=system_instruction,
                                ttl=f"{self.valves.RAG_CACHE_TTL_SECONDS}s",
                            ...
                            if self._rag_cache_expire_time is None:
                                self._rag_cache_expire_time = (
                                    now + self.valves.RAG_CACHE_TTL_SECONDS
                                )
                            self._rag_cache_key = context_hash
                            cached_content_name = self._rag_cache_name
                            system_instruction = None
                            self.log.debug(
                                f"RAG: created context cache {self._rag_cache_name}, expires in {self.valves.RAG_CACHE_TTL_SECONDS}s"
                            )
                        except Exception as e:
                            self.log.warning(
                                f"RAG: context cache create failed, using full context: {e}"
                            )
            ...
            generation_config = self._configure_generation(
                ...
                model_id,
                cached_content=cached_content_name if not supports_image_generation else None,
            )
            ...
            if stream:
                ...
                if supports_image_generation:
                    ...
                    try:
                        ...
                        if (
                            cached_content_name
                            and system_instruction_backup
                            and not supports_image_generation
                            and self._is_rag_cache_invalid_error(e)
                        ):
                            new_name = await self._recreate_rag_cache_after_invalid(
                                system_instruction_backup, model_id
                            )
                            if new_name:
                                generation_config = self._configure_generation(
                                    body,
                                ...
                                try:
                                    ...
                                    self.log.exception(
                                        f"Error in streaming retry {request_id}: {e2}"
                                    )
                                    return f"Error during streaming: {e2}"
                        self.log.exception(
                    ...
                    try:
                        ...
                        if (
                            ...
                            if new_name:
                                ...
                                self.log.debug(
                                    f"RAG: retrying non-streaming after cache recreate"
                                )
                                response = await self._retry_with_backoff(get_response)
                            else:
                                raise
